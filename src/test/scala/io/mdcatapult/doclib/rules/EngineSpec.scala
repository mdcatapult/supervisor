...
class EngineSpec extends TestKit(ActorSystem("EngineSpec", ConfigFactory.parseString(
  ...
  "A PDF which has been raw text, pdf intermediates and bounding box processed with intermediates reset" should {
    "return a pdf_intermediates sendable" in {
      val flags = List(
        DoclibFlag(
          key = "rawtext",
          version = ConsumerVersion(
  ...
  "A Tabular doc with completed ner with one reset and a reset but unfinished analysis" should {
    "return 1 analysis sendable" in {
      val doc: DoclibDoc = dummy.copy(
        mimetype = "text/tab-separated-values",
        source = "/dummy/path/to/dummy/file",
        doclib = List(
  ...
  "A Tabular doc with completed ner with one reset and analysis reset before started time" should {
    "return no sendables" in {
      val doc: DoclibDoc = dummy.copy(
        mimetype = "text/tab-separated-values",
        source = "/dummy/path/to/dummy/file",
        doclib = List(
